export function formatTime(date = new Date(), format = 'F') {
	return `<t:${Math.floor(date.getTime() / 1000)}:${format}>`;
}

export async function awaitSelection(ctx, choices, msg, options = { min_values: 1, max_values: 1, placeholder: '- - -' }, ephemeral) {
	var components = [{
		type: 3,
		custom_id: 'selector',
		options: choices,
...
export async function getChoice(bot, msg, user, time, update = true) {
